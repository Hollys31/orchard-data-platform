...
(function (global, factory) {
  ...
  var pathPrefix = './ezuikit/'
  var jqueryJS = pathPrefix + 'jquery.min.js';
  var ckplayerJS = pathPrefix + 'ckplayer/ckplayer.js';
  var ckplayerSWF = pathPrefix + 'ckplayer/ckplayer.swf';
  var m3u8SWF = pathPrefix + 'ckplayer/m3u8.swf';
  var flv_js = pathPrefix + 'flv.min.js';
  var hlsJS = pathPrefix + 'hls.min.js';
  var mpegJS = pathPrefix + 'jsmpeg.min.js';
  var wav = pathPrefix + 'wav-audio-encoder.js';
  ...
  var EZUIPlayer = function (playParams) {
    ...
    if (typeof playParams === 'object' && playParams.decoderPath) {
      ...
      console.log('getRealUrl.....', getRealUrl)
      /**是否自动播放 */
      ...
      if (/\.m3u8/.test(playParams)) {
        ...
        * */
        if (!ltIE11()) {
          ...
          console.log('WS协议不支持Ie11以下的浏览器！请使用IE11，或者更高版本的浏览器');
          return;
        }
